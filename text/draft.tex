...
There were X automatic matches (call them ``suggestions''). The quality
of these suggestions was then evaluated by random sampling. A suggestion
is considered to be correct if and only if the meaning of the keyword
from \texttt{keywords/keywords\_clean\_histogram.json} corresponds to
the meaning of the suggested subject term from Wikidata. Note that for
homonymous or polysemous keywords, it is impossible to confirm a correct
match without further context; those keywords therefore cannot be
reconciled (but see \protect\hyperlink{construction}{section
``Construction''} for a possible solution).

Unfortunately, random sampling showed that the overall quality of the
suggestions was not satisfactory. A two-pronged strategy was adopted to
ameliorate the quality of the reonciliation:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  manual matching of the top results
\item
  automated processes for the rest
\end{enumerate}

!! calculate for each of annif yso mesh how many percent of keywords or
total occurrences are covered by reonciliated data.
